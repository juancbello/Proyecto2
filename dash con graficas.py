...
fig1 = px.histogram(df[df['num'] != 0], x='age', color='sex', barmode='group', marginal='box', nbins=20,
...
fig2 = px.scatter_matrix(df, dimensions=['fbs', 'chol', 'trestbps'], color='num', opacity=0.7,
                         labels={'fbs': 'Fasting Blood Sugar', 'chol': 'Cholesterol', 'trestbps': 'Resting Blood Pressure', 'num': 'Num'})


# GRAFICA ANGINAL
fig3 = px.histogram(df, x='cp', color='exang', barmode='group', nbins=5,
...
tab1_content = html.Div([
    ...
    html.Br(),
    html.P('En el siguiente grafico, realizamos un conteo de personas que padecen de complicaciones cardiaca. La personas se encuentran agrupadas por edad y sexo'),
    html.Br(), 
    html.P('Es importante mencionar que la edad se encuentra dividida en 10 intervalos de igual proporcion.'),
    html.Br(),  
